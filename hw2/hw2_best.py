...
"""
x_data = x_data.astype(float).as_matrix()
...
#sel_array = np.array(occupation)
#capital_gain,education
sel_array = np.concatenate((age,capital_gain,capital_loss,hours_per_week,workclass,education_num,education,marital_status,occupation,country))
x_data = x_data[:,sel_array]
...
lrn =  np.ones((1,2*len(sel_array)+1))/100
...
for i in range(iteration):
	...
	if i%100 == 0:
		...
		print(Loss)
		err_train_win = np.sum((f_win<=threshold).astype(int))
		err_train_lose = np.sum((f_lose>threshold).astype(int))
		err_train = err_train_win + err_train_lose
		print("Count: %d + %d | Error Rate: %f" %(err_train_win,err_train_lose,err_train/num_total))

	err_win = np.sum(-x_win*((1 - f_win).reshape(-1,1)), axis = 0)
	err_lose = np.sum(-x_lose*((-f_lose).reshape(-1,1)), axis = 0)

	grad_w = err_win + err_lose + 2*reg*w
	lrn = lrn + grad_w ** 2
...
# Save Model
np.save('logistic_test_w.npy',w)

############################################################################

# Read Model
w = np.load('logistic_test_w.npy')
"""
